...
export class Renderer {
  ...
  async serialize(requestUrl: string, isMobile: boolean):
    ...
    function stripPage() {
      ...
      let elements = document.querySelectorAll('script:not([type]), script[type*="javascript"], link[rel=import]');
    ...
    try {
      ...
      response = await page.goto(
        requestUrl, { timeout: this.config.timeout, waitUntil: ['load', 'domcontentloaded', 'networkidle0', 'networkidle2'] });
      // await page.click('.optanon-alert-box-bg > .optanon-alert-box-button-container > .optanon-alert-box-button > .optanon-alert-box-button-middle > .optanon-allow-all')
    } catch (e) {
      console.error(e);
    }
    ...
    await page.evaluate((cookieWarningFilters) => {
      cookieWarningFilters.forEach((cookieFilter: string) => {
        let body = document.querySelector("body");
        let cookieBox = body!.querySelector(cookieFilter);
        if (cookieBox) {
          cookieBox.remove();
